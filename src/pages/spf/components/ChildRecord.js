...
export default class ChildRecord extends Component {
  ...
  render() {
    ...
    return (
      <ChildRecordPanel collapsed={ collapsed } childrenCollapsed={ this.state.childrenCollapsed } type={ record.displayType } value={ record.value } record={ record.record } toggle={ () => this.toggleChildren() }>
        { this.renderChildren() }
      </ChildRecordPanel>
    );
  }
